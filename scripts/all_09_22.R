...
negativeCiliary<-hgncConverter2(negativeCiliary, "Gene_name")
...
orthology<-hgncConverter2(orthology, "Gene2Symbol")
...
hgnc_mouse<-fread("files/hgnc_names.txt") %>% 
  ...
  dplyr::select(1,3) %>% 
  hgncConverter2("Approved symbol")
...
cele_ids<-fread("https://downloads.wormbase.org/releases/WS285/species/c_elegans/PRJNA13758/annotation/c_elegans.PRJNA13758.WS285.geneIDs.txt.gz") %>% 
  ...
  left_join(cele_orthology[,c(1,3)], by = "Gene1Symbol")
...
drosophila<-rbind(drosophila, onlyortdro[,c(1,3)]) %>% unique() %>% hgncConverter2("Gene2Symbol")
...
rocs <- roc(is.ciliary ~ interaction_scores + phylogenetic_scores + motif_score +
              pa_score + single_cell_score + total_score_sums, data = all_scores1)

lbls<-c("Interactions", "Comparative genomics", "TF binding", "Text mining", "scRNA-seq", "CilioGenics")
#ggroc(rocs)
...
ggsave("roc_all_bw.jpg", width = 21, height = 14, dpi = 300) 
...
ggnet2(nw, color = "is.ciliary", node.size = 5, label = c("CC2D2A", "IFT74"), label.size = 5,
       palette = c("Ciliary" = "steelblue", "Non-ciliary" = "tomato", "Unknown" = "palegreen", "Candidate" = "yellow2"),
       mode = "kamadakawai") +
  guides(color=guide_legend(title="Type", override.aes = list(size=7))) +
  theme(legend.title = element_text(size=15),
        legend.text = element_text(size=12))

ggsave("interaction_plot_IFT74_CC2D2A_2.jpg", width = 12, height = 8, dpi = 300) 
...
ggsave("interaction_plot_candidates_all2.jpg", width = 12, height = 8, dpi = 300) 
