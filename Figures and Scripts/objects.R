...
mouse_orthology<-fread("HGNC_AllianceHomology.rpt")
...
mouse_synonyms<-fread("MRK_List2.rpt")
...
hgnc_mouse<-fread("hgnc_names.txt") %>% 
...
cele_ids<-fread("https://downloads.wormbase.org/releases/current-development-release/species/c_elegans/PRJNA13758/annotation/c_elegans.PRJNA13758.WS284.geneIDs.txt.gz") %>% 
  mutate(V3 = ifelse(V3 == "", V4, V3)) %>%
  rename("Gene1Symbol" = "V3", "Gene_ID" = "V2") %>% 
  select(Gene1Symbol, Gene_ID) %>% 
